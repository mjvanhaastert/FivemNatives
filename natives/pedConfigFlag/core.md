# Ped Config Flags (`SetPedConfigFlag`)
...
- Native type: client en server.
- Doel: low-level ped gedrag aan/uit zetten via interne flags (per ped).
- Gedrag: flags blijven actief op die ped tot je ze zelf terugzet of de ped verdwijnt.

## Waarom deze native gebruiken i.p.v. vergelijkbare natives

Gebruik `SetPedConfigFlag` wanneer je **fijnmazige, engine-level ped states** wilt sturen die niet goed af te dekken zijn met 1 losse helper-native.

Sterke punten t.o.v. vergelijkbare opties:

- **Fijnmazig per gedrag**: je toggelt exact 1 intern gedrag (bijv. lock-on, ragdoll reactie, voertuiggedrag).
- **Combineren van gedrag**: meerdere flags tegelijk maken complexe AI/protection profiles mogelijk.
- **Consistent per ped state**: werkt als state-bit op de ped i.p.v. alleen een eenmalige task call.
- **Breed inzetbaar**: combat, movement, vehicle, targeting, reactions, constraints.

Wanneer liever andere natives:

- Als er een duidelijke high-level native bestaat voor exact je doel, is die vaak leesbaarder (bijv. `SetEntityInvincible`, `SetBlockingOfNonTemporaryEvents`, `SetPedCanRagdoll`, `SetPedCombatAttributes`).
- Voor tijdelijke task flow gebruik je eerder task natives; flags zijn meer “state policy”.

## Praktische richtlijnen

- Zet alleen flags die je echt nodig hebt (overmatige flags geven onvoorspelbare combinaties).
- Documenteer per flag waarom hij aan/uit staat.
- Reset flags expliciet bij cleanup/despawn om side effects te voorkomen.
- Test in network context: sommige effecten lijken anders afhankelijk van ownership/sync.
